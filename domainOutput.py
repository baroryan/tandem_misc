...
class mesh:
    def __init__(self, file_path):
        ...
        self.mesh = None
        self.physical_curves = {}  # Mapping: physical id -> list of line segments (each segment is an array of point indices)
        self.phys_id_to_name = {}  # Mapping: physical id -> physical group name (if available)
        self.read_mesh()
    ...
    def read_mesh(self):
        ...
        Returns:
            tuple: (mesh, physical_curves, phys_id_to_name)
        """
        self.mesh = meshio.read(self.file_path)

        # Build mapping from physical id to name for 1D (curve) groups.
        if self.mesh.field_data:
            for name, (phys_id, dim) in self.mesh.field_data.items():
                if dim == 1:  # Only consider 1D entities (physical curves)
                    self.phys_id_to_name[phys_id] = name

        # Extract physical curves from the cell data.
        if "gmsh:physical" in self.mesh.cell_data:
            for cell_block, phys_data in zip(self.mesh.cells, self.mesh.cell_data["gmsh:physical"]):
                if cell_block.type == "line":
                    for seg, phys in zip(cell_block.data, phys_data):
                        self.physical_curves.setdefault(phys, []).append(seg)
        else:
            print("No physical data found in the mesh file.")

        return self.mesh, self.physical_curves, self.phys_id_to_name

    def plot_physical_curves(self, ax=None,show_triangles=True):
        """
        Plots the physical curves extracted from the mesh.
        Optionally overlays the mesh triangles in very light grey.

        Parameters:
            show_triangles (bool): If True, plots the mesh triangles in light grey.
        """
        ...
        # Optionally plot triangles in light grey.
        if show_triangles:
            for cell_block in self.mesh.cells:
                if cell_block.type == "triangle":
                    triangles = self.mesh.points[cell_block.data]
                    for tri in triangles:
                        pts = tri[:, :2]  # use only x and y coordinates
                        pts = np.vstack([pts, pts[0]])  # close the triangle loop
                        ax.plot(pts[:, 0], pts[:, 1], color="lightgrey", linewidth=0.5)

        # Set up a colormap for distinct colors.
        cmap = plt.get_cmap("tab10")

        # Plot each physical curve.
        for i, (phys, segs) in enumerate(self.physical_curves.items()):
            ...
            for j, seg in enumerate(segs):
                pts = self.mesh.points[seg, :2]  # retrieve x and y coordinates
                if j == 0:
